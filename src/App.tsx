    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-white/80 dark:bg-black/80 backdrop-blur-xl border-b border-black/5 dark:border-white/10 py-3" : "bg-transparent py-6"}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <a href="#" className="flex items-center group cursor-pointer">
          <img 
            src="https://images.squarespace-cdn.com/content/v1/54a68da3e4b0c309d017934f/3e00287e-384b-4e4b-8a5b-3877ea44dc50/NH_Brand+%281%29-13.png?format=1500w" 
            alt="Nik Hannay" 
            className="h-6 md:h-8 w-auto invert dark:invert-0 transition-all duration-300 group-hover:opacity-80"
...
const BrandLogo = ({ name, domain, simpleSlug, className = "h-8" }: { name: string, domain: string, simpleSlug: string, className?: string }) => {
  ...
  return (
    <div className="relative flex items-center justify-center min-w-[160px] h-12 group">
      {src && (
        <img 
          src={src} 
          alt={name} 
          className={`${className} w-auto object-contain grayscale transition-all duration-700 group-hover:grayscale-0 ${loaded ? 'opacity-60 group-hover:opacity-100' : 'opacity-0'}`} 
      ...
      {(!loaded || !src) && (
        <span className="absolute inset-0 flex items-center justify-center text-[10px] font-mono font-bold text-zinc-300 dark:text-zinc-700 uppercase tracking-[0.3em]">
          {name}
        </span>
      )}
    </div>
      ...
      >
        ...
        >
          ...
          <h1 className="text-6xl md:text-8xl lg:text-9xl font-serif font-light tracking-tight text-black dark:text-white mb-10 leading-[0.9] text-balance">
            Design <br />
            <span className="text-accent italic">Director</span>
          </h1>
          ...
          >
            {[
              { name: "McLaren", domain: "mclaren.com", simpleSlug: "mclaren", className: "h-12" },
              { name: "Vodafone", domain: "vodafone.com", simpleSlug: "vodafone" },
              { name: "Jaguar", domain: "jaguarlandrover.com", simpleSlug: "jaguar" },
              { name: "BP", domain: "bp.com", simpleSlug: "bp" },
              { name: "EDF", domain: "edfenergy.com", simpleSlug: "edf" },
              { name: "NAB", domain: "nab.com.au", simpleSlug: "nationalaustraliabank" }
            ].map((brand) => (
              <BrandLogo 
                key={brand.name} 
                name={brand.name} 
...
const ProjectCard = ({ title, category, image, tags, onClick }: { title: string, category: string, image: string, tags: string[], onClick: () => void }) => (
  <motion.div 
    variants={{
...
const Work = ({ onProjectClick }: { onProjectClick: (_project: Project) => void }) => {
  ...
  return (
    <section id="work" className="py-40 bg-white dark:bg-black">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          {projects.map((p, i) => (
            <ProjectCard key={i} {...p} onClick={() => onProjectClick(p)} />
          ))}
        </motion.div>
      </div>
    </section>
        ...
        >
          <div className="lg:col-span-7">
            ...
            <div className="space-y-8 text-zinc-600 dark:text-zinc-400 text-lg md:text-xl font-light leading-relaxed">
              <p>
                I’m a Design Director specialising in building <span className="text-black dark:text-white font-normal">scalable digital products</span> and <span className="text-black dark:text-white font-normal">high-performing design teams</span>.
              </p>
              <p>
                Over the past 15+ years I’ve helped organisations transform complex ideas into intuitive digital experiences – from Enterprise Saas platforms to consumer products.
              </p>
            ...
            <div className="mt-20 grid grid-cols-2 sm:grid-cols-3 gap-12">
              ...
              <div>
                <p className="text-5xl font-serif font-light text-black dark:text-white mb-2">12</p>
                <p className="text-[10px] font-mono text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">Global Awards</p>
              </div>
            </div>
          </div>
